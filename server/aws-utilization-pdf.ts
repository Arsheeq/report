...
export async function generateAwsUtilizationPDF({
  ...
  try {
    ...
    const filename = `${safeAccountName}-utilization-report-${formattedDate}.${format}`;
...
function generateCoverPage(doc: PDFKit.PDFDocument, options: {
  ...
  // Account name as prominent header
  doc.fontSize(24)
     .font('Helvetica-Bold')
     .text(`Account: ${options.accountName}`, {
       align: 'center'
     })
     .moveDown(1);
  ...
  doc.rect(tableLeft, tableTop, columnWidth * 2, rowHeight * 4) //Reduced rows
     .fill('#F8F9FA');
